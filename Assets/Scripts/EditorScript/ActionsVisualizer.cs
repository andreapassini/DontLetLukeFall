...
// This script is to visualize actions while creating levels; this script should be disabled in the final game
{
    ...
    [FormerlySerializedAs("_visualizeActions")] [SerializeField] private bool _visualizeActionsInPlay; // Chose if you want to visualize actions or not
    ...
    private void Start()
    {
        StartCoroutine(StartAfterALittleWhile()); // Start after a little while so that it can be loaded the _actionsTypeSequence
    }
    
    IEnumerator StartAfterALittleWhile()
    {
        yield return new WaitForSeconds(0.1f);
        if (_visualizeActionsInPlay == true)
        {
